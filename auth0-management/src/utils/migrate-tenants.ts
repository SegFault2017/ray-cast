...
const LEGACY_TENANTS = [
  { key: "dev", name: "Development", environment: "Dev", color: Color.Green },
  { key: "staging", name: "Staging", environment: "Staging", color: Color.Orange },
  { key: "prod", name: "Production", environment: "Prod", color: Color.Red },
] as const;
...
export async function migrateTenants(): Promise<void> {
  ...
  }).map(({ key, name, environment, color }) => ({
    id: key,
    name,
    environment,
    color,
